...
# ============================================================================
# C.5.1 SemanticStructure Analysis - Image + Snapshot (Parameterset 5)
# ============================================================================
Write-Host "`n[C.5.1] SemanticStructure - Parameterset 5 (imagePath + snapshotjson)" -ForegroundColor Yellow

try {
  $r = Invoke-ToolReport -Spec $CVA -ToolName 'cognitive_visual_semanticstructure' -ToolArgs @{imagePath=$BASELINE_IMG; snapshotjson=$SNAPSHOT_JSON} -Filename $REPORT_NAME
  Write-Host "  ✓ C.5.1 Image + Snapshot: $($r.path)" -ForegroundColor Green
  $results += @{Test="C.5.1 SemanticStructure - Image+Snapshot"; Status="PASS"; Report=$r.path}
} catch {
  Write-Host "  ✗ C.5.1 Error: $_" -ForegroundColor Red
  $results += @{Test="C.5.1 SemanticStructure - Image+Snapshot"; Status="FAIL"; Error=$_.Exception.Message}
}
