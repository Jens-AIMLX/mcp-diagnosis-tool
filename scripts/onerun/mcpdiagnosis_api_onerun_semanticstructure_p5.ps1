...
$BASELINE_IMG_NAME = "semanticstructure_p5_$TIMESTAMP.jpg"
$BASELINE_IMG = "$SCREENSHOT_DIR/$BASELINE_IMG_NAME"
$REPORT_NAME = "MCPDiagnosis_Report_semanticstructure_p5_$TIMESTAMP.md"
...
Write-Host "Test: cognitive_visual_semanticstructure - Parameterset 5" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
# ============================================================================
# C.5.5 SemanticStructure Analysis - Image + JSON (Parameterset 5)
# ============================================================================
Write-Host "`n[C.5.5] SemanticStructure - Parameterset 5 (imagePath + snapshotjson)" -ForegroundColor Yellow

try {
  $r = Invoke-ToolReport -Spec $CVA -ToolName 'cognitive_visual_semanticstructure' -ToolArgs @{imagePath=$BASELINE_IMG; snapshotjson='C:/Users/jenss/OneDrive - Singularyt UG/Code/Test/.evidence/snapshots/snapshot.json'} -Filename $REPORT_NAME
  Write-Host "  ✓ C.5.5 Image + JSON: $($r.path)" -ForegroundColor Green
  $results += @{Test="C.5.5 SemanticStructure - JSON"; Status="PASS"; Report=$r.path}
} catch {
  Write-Host "  ✗ C.5.5 Error: $_" -ForegroundColor Red
  $results += @{Test="C.5.5 SemanticStructure - JSON"; Status="FAIL"; Error=$_.Exception.Message}
}
