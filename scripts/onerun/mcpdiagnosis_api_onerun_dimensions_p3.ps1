...
$BASELINE_IMG_NAME = "dimensions_p3_$TIMESTAMP.jpg"
$BASELINE_IMG = "$SCREENSHOT_DIR/$BASELINE_IMG_NAME"
$REPORT_NAME = "MCPDiagnosis_Report_dimensions_p3_$TIMESTAMP.md"
...
Write-Host "Test: cognitive_visual_dimensions - Parameterset 3" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
# ============================================================================
# C.2.3 Dimensions Analysis - Image Mode - Area Focus (Parameterset 3)
# ============================================================================
Write-Host "`n[C.2.3] Dimensions - Parameterset 3 (area focus)" -ForegroundColor Yellow

try {
  $r = Invoke-ToolReport -Spec $CVA -ToolName 'cognitive_visual_dimensions' -ToolArgs @{imagePath=$BASELINE_IMG; area='Zertifikate Verwaltung'} -Filename $REPORT_NAME
  Write-Host "  ✓ C.2.3 Image Mode (area): $($r.path)" -ForegroundColor Green
  $results += @{Test="C.2.3 Dimensions - Area"; Status="PASS"; Report=$r.path}
} catch {
  Write-Host "  ✗ C.2.3 Error: $_" -ForegroundColor Red
  $results += @{Test="C.2.3 Dimensions - Area"; Status="FAIL"; Error=$_.Exception.Message}
}
