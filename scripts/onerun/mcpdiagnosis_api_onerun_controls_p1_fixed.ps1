...
$BASELINE_IMG_NAME = "dimensions_p1_$TIMESTAMP.jpg"
$BASELINE_IMG = "$SCREENSHOT_DIR/$BASELINE_IMG_NAME"
$REPORT_NAME = "MCPDiagnosis_Report_dimensions_p1_$TIMESTAMP.md"
...
Write-Host "Test: cognitive_visual_dimensions - Parameterset 1" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
# ============================================================================
# C.2.1 Dimensions Analysis - Image Mode - Latest (Parameterset 1)
# ============================================================================
Write-Host "`n[C.2.1] Dimensions - Parameterset 1 (no parameters)" -ForegroundColor Yellow

try {
  $r = Invoke-ToolReport -Spec $CVA -ToolName 'cognitive_visual_dimensions' -Filename $REPORT_NAME
  Write-Host "  ✓ C.2.1 Image Mode (latest): $($r.path)" -ForegroundColor Green
  $results += @{Test="C.2.1 Dimensions - Latest"; Status="PASS"; Report=$r.path}
} catch {
  Write-Host "  ✗ C.2.1 Error: $_" -ForegroundColor Red
  $results += @{Test="C.2.1 Dimensions - Latest"; Status="FAIL"; Error=$_.Exception.Message}
}
