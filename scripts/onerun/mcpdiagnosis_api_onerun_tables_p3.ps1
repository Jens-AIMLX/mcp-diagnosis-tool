...
$BASELINE_IMG_NAME = "tables_p3_$TIMESTAMP.jpg"
$BASELINE_IMG = "$SCREENSHOT_DIR/$BASELINE_IMG_NAME"
$REPORT_NAME = "MCPDiagnosis_Report_tables_p3_$TIMESTAMP.md"
...
Write-Host "Test: cognitive_visual_tables - Parameterset 3" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
# ============================================================================
# C.4.3 Tables Analysis - Image Mode - Area Focus (Parameterset 3)
# ============================================================================
Write-Host "`n[C.4.3] Tables - Parameterset 3 (area focus)" -ForegroundColor Yellow

try {
  $r = Invoke-ToolReport -Spec $CVA -ToolName 'cognitive_visual_tables' -ToolArgs @{imagePath=$BASELINE_IMG; area='Zertifikate Verwaltung'} -Filename $REPORT_NAME
  Write-Host "  ✓ C.4.3 Image Mode (area): $($r.path)" -ForegroundColor Green
  $results += @{Test="C.4.3 Tables - Area"; Status="PASS"; Report=$r.path}
} catch {
  Write-Host "  ✗ C.4.3 Error: $_" -ForegroundColor Red
  $results += @{Test="C.4.3 Tables - Area"; Status="FAIL"; Error=$_.Exception.Message}
}
