...
$BASELINE_IMG_NAME = "controls_p3_$TIMESTAMP.jpg"
$BASELINE_IMG = "$SCREENSHOT_DIR/$BASELINE_IMG_NAME"
$REPORT_NAME = "MCPDiagnosis_Report_controls_p3_$TIMESTAMP.md"
...
Write-Host "Test: cognitive_visual_controls - Parameterset 3" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
# ============================================================================
# C.3.3 Controls Analysis - Image Mode - Area Focus (Parameterset 3)
# ============================================================================
Write-Host "`n[C.3.3] Controls - Parameterset 3 (area focus)" -ForegroundColor Yellow

try {
  $r = Invoke-ToolReport -Spec $CVA -ToolName 'cognitive_visual_controls' -ToolArgs @{imagePath=$BASELINE_IMG; area='Zertifikate Verwaltung'} -Filename $REPORT_NAME
  Write-Host "  ✓ C.3.3 Image Mode (area): $($r.path)" -ForegroundColor Green
  $results += @{Test="C.3.3 Controls - Area"; Status="PASS"; Report=$r.path}
} catch {
  Write-Host "  ✗ C.3.3 Error: $_" -ForegroundColor Red
  $results += @{Test="C.3.3 Controls - Area"; Status="FAIL"; Error=$_.Exception.Message}
}
