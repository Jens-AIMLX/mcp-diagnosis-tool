...
$BASELINE_IMG_NAME = "controls_p4_$TIMESTAMP.jpg"
$BASELINE_IMG = "$SCREENSHOT_DIR/$BASELINE_IMG_NAME"
$REPORT_NAME = "MCPDiagnosis_Report_controls_p4_$TIMESTAMP.md"
...
Write-Host "Test: cognitive_visual_controls - Parameterset 4" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
# ============================================================================
# C.3.4 Controls Analysis - URL Mode (Parameterset 4)
# ============================================================================
Write-Host "`n[C.3.4] Controls - Parameterset 4 (URL mode)" -ForegroundColor Yellow

try {
  $r = Invoke-ToolReport -Spec $CVA -ToolName 'cognitive_visual_controls' -ToolArgs @{url='http://localhost:3060'} -Filename $REPORT_NAME
  Write-Host "  ✓ C.3.4 URL Mode: $($r.path)" -ForegroundColor Green
  $results += @{Test="C.3.4 Controls - URL"; Status="PASS"; Report=$r.path}
} catch {
  Write-Host "  ✗ C.3.4 Error: $_" -ForegroundColor Red
  $results += @{Test="C.3.4 Controls - URL"; Status="FAIL"; Error=$_.Exception.Message}
}
